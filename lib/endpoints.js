...
app.get(`${ config.baseUrl }admin/login`, (req, res) => {
  if (req.session.adminAuthenticated) return res.redirect(`${ config.baseUrl }admin`);
  res.send(loginPage({ ...pugVars, error: null }));
});

app.post(`${ config.baseUrl }admin/login`, (req, res) => {
  const { username, password } = req.body;
  if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD && ADMIN_PASSWORD) {
    req.session.adminAuthenticated = true;
    return res.redirect(`${ config.baseUrl }admin`);
  }
  res.send(loginPage({ ...pugVars, error: 'Invalid credentials' }));
});
