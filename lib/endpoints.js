...
// The admin panel is enabled when a usable login method exists; credentials
// live in oauth.json (generated on first launch), never in the environment
const adminEnabled = () => oidc.passwordLoginEnabled() || oidc.enabled;
const SECRET_KEY = (() => {
...
// Hashed frontend bundle names from the Vite build manifest. Re-read when the
// manifest changes so a rebuild lands without a backend restart. Without a
// manifest (source checkout that was never built) fall back to the un-hashed
// names so the old bundle still loads.
const viteManifestPath = path.join(__dirname, '../public/app/.vite/manifest.json');
let viteAssetsCache = { mtime: -1, entries: null };
function viteAssets() {
  let mtime = 0;
  try { mtime = fs.statSync(viteManifestPath).mtimeMs; } catch { /* no build */ }
  if (viteAssetsCache.entries && viteAssetsCache.mtime === mtime) return viteAssetsCache.entries;
  let manifest = {};
  if (mtime) {
    try { manifest = JSON.parse(fs.readFileSync(viteManifestPath, 'utf8')); } catch { /* mid-write */ }
  }
  // An entry's CSS can also live on the chunks it imports
  const cssOf = (key, seen = new Set()) => {
    const chunk = manifest[key];
    if (!chunk || seen.has(key)) return [];
    seen.add(key);
    return (chunk.imports || []).flatMap(i => cssOf(i, seen)).concat(chunk.css || []);
  };
  const entries = {};
  for (const name of ['upload', 'download', 'admin']) {
    const chunk = manifest[`src/${ name }.js`];
    entries[name] = chunk
      ? { js: chunk.file, css: [...new Set(cssOf(`src/${ name }.js`))] }
      : { js: `${ name }.js`, css: [] };
  }
  viteAssetsCache = { mtime, entries };
  return entries;
}
...
const pugVars = {
  ...
  appVersion,
  // Getter so every `...pugVars` spread picks up a fresh manifest read
  get assets() { return viteAssets(); },
};

const clientUa = req => (req.get('user-agent') || '').slice(0, 160) || undefined;
...
eventBus.on('fileUploaded', upload => activityLog.record('upload', {
  ...
  ua: upload.metadata.clientUa,
}));
eventBus.on('fileDownloaded', e => activityLog.record('download', { sid: e.sid, file: e.file, ip: e.ip, ua: e.ua }));
eventBus.on('archiveDownloaded', e => activityLog.record('archive', { sid: e.sid, file: e.file, ip: e.ip, ua: e.ua }));
...
app.use(require('express-session')({
  ...
  cookie: { httpOnly: true, sameSite: 'lax', maxAge: oidc.sessionDays * 864e5 },
}));
...
// Static files. Bundle files carry a content hash in their name (see
// viteAssets), so those cache forever; the un-hashed fallback names and
// everything under assets/ must keep revalidating (styles.css and the
// favicons are busted with ?v=appVersion instead, fonts never change).
const hashedName = /-[\w-]{8}\.\w+$/;
app.use(`${ config.baseUrl }app`, express.static(path.join(__dirname, '../public/app'), {
  setHeaders(res, filePath) {
    if (hashedName.test(filePath)) res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
  }
}));
app.use(`${ config.baseUrl }assets`, express.static(path.join(__dirname, '../public/assets'), { maxAge: '7d' }));
...
const loginVars = error => ({
  ...pugVars,
  error,
  passwordEnabled: oidc.passwordLoginEnabled(),
  oidcEnabled: oidc.enabled,
});

// Audit trail entry with the request's actor, client IP and user agent.
// Event-specific fields (attempted username, changed settings, sid) come
// via data and may override the session actor.
function audit(req, type, data = {}) {
  auditLog.record(type, {
    user: (req.session && req.session.adminUser) || undefined,
    ip: req.ip,
    ua: clientUa(req),
    ...data,
  });
...
app.get(`${ config.baseUrl }admin/login`, (req, res, next) => {
  if (!adminEnabled()) return next();
  if (req.session.adminAuthenticated) return res.redirect(`${ config.baseUrl }admin`);
  res.send(loginPage(loginVars(null)));
});

app.post(`${ config.baseUrl }admin/login`, async (req, res, next) => {
  if (!adminEnabled()) return next();
  const { username, password } = req.body || {};
  const cfg = oidc.readConfig();
  if (oidc.passwordLoginEnabled()
    && safeEqual(username, cfg.admin_username)
    && await verifyPassword(cfg.admin_password_hash, String(password || ''))) {
    audit(req, 'login', { user: String(username) });
    return createAdminSession(req, res, next, String(username));
  }
  audit(req, 'login_failed', { user: String(username || '') });
  res.status(401).send(loginPage(loginVars('Invalid credentials')));
});

const oidcCallbackUri = req => {
  // Prefer the configured external URL; fall back to the request host
  const ext = oidc.readConfig().external_url.replace(/\/+$/, '');
  const origin = ext || `${ req.protocol }://${ req.get('host') }`;
  return `${ origin }${ config.baseUrl }admin/oidc/callback`;
};
...
app.get(`${ config.baseUrl }admin/oidc/login`, async (req, res, next) => {
  ...
  try {
    ...
    audit(req, 'oidc_failed', { error: e.message });
...
app.get(`${ config.baseUrl }admin/oidc/callback`, async (req, res, next) => {
  ...
  try {
    ...
    audit(req, 'oidc_login', { user: user.name, sub: user.sub });
...
app.post(`${ config.baseUrl }admin/logout`, (req, res) => {
  if (req.session.adminAuthenticated) audit(req, 'logout');
...
function adminApi(req, res, next) {
  if (!adminEnabled()) return next(httpErrors.NotFound());
...
app.get(`${ config.baseUrl }admin/activity.json`, adminApi, (req, res) => {
  ...
  res.json({ events: activityLog.tail(limit) });
});

// OIDC settings, stored in oauth.json in the upload dir and edited from the
// admin panel's Authentication section. Same fields and restart-to-apply
// semantics as multi-downloader. The secret is write-only.
app.get(`${ config.baseUrl }admin/auth-config.json`, adminApi, (req, res) => {
  const cfg = oidc.readConfig();
  res.json({
    enabled: cfg.enabled,
    enabled_runtime: oidc.enabled,
...
app.post(`${ config.baseUrl }admin/auth-config.json`, adminApi, async (req, res) => {
  const body = req.body || {};
  const cfg = oidc.readConfig();
  // snapshot before mutating so the audit entry can name the changed fields
  const prev = { ...cfg };
  cfg.enabled = !!body.enabled;
  cfg.discovery_url = String(body.discovery_url || '').trim();
  cfg.client_id = String(body.client_id || '').trim();
  if (body.client_secret) cfg.client_secret = String(body.client_secret).trim();
  cfg.session_lifetime_days = Math.max(1, Math.min(365, parseInt(body.session_lifetime_days, 10) || 7));
  cfg.external_url = String(body.external_url || '').trim().replace(/\/+$/, '');
  cfg.password_login = !!body.password_login;
  if (String(body.admin_username || '').trim()) {
    cfg.admin_username = String(body.admin_username).trim();
  }
  ...
  if (newPassword) {
    ...
    cfg.admin_password_hash = await hashPassword(newPassword);
    cfg.must_change_password = false;
  }

  if (cfg.enabled && !(cfg.discovery_url && cfg.client_id && cfg.client_secret)) {
    return res.status(400).json({
      message: 'Discovery URL, client ID and client secret are all required to enable OIDC',
    });
  }
  if (!cfg.enabled && !cfg.password_login) {
    return res.status(400).json({ message: 'At least one login method must stay enabled' });
  }
  if (!cfg.password_login && !oidc.enabled) {
    ...
    return res.status(400).json({
      message: 'Enable OIDC and restart the container before disabling password login',
    });
  }
  ...
  try {
    ...
    console.error(e);
    return res.status(500).json({ message: 'Failed to write oauth.json' });
  }

  // Field names only, never values: the secret and password must not
  // land in the log
  const changed = [];
  if (cfg.enabled !== prev.enabled) changed.push('oidc_enabled');
  if (cfg.discovery_url !== prev.discovery_url) changed.push('discovery_url');
  if (cfg.client_id !== prev.client_id) changed.push('client_id');
  if (cfg.client_secret !== prev.client_secret) changed.push('client_secret');
  if (cfg.external_url !== prev.external_url) changed.push('external_url');
  if (cfg.session_lifetime_days !== prev.session_lifetime_days) changed.push('session_lifetime_days');
  if (cfg.password_login !== prev.password_login) changed.push('password_login');
  if (cfg.admin_username !== prev.admin_username) changed.push('admin_username');
  if (newPassword) changed.push('admin_password');
  if (changed.length) audit(req, 'settings_changed', { changed });

  res.json({ ok: true, restart_required: true });
});

app.get(`${ config.baseUrl }admin/audit.json`, adminApi, (req, res) => {
  res.json({ events: auditLog.tail(200) });
});
...
app.delete(`${ config.baseUrl }admin/files/:sid/:key`, adminApi, async (req, res) => {
  ...
  try {
    ...
    audit(req, 'file_deleted', { sid, file: file.metadata.name });
    res.status(204).end();
  } catch (e) {
    console.error(e);
    res.status(500).end();
...
app.delete(`${ config.baseUrl }admin/files/:sid`, adminApi, async (req, res) => {
  ...
  try {
    ...
    audit(req, 'share_deleted', { sid, files: count });
    res.status(204).end();
  } catch (e) {
    console.error(e);
    res.status(500).end();
